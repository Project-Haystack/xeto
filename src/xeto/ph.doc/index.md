- [Intro](Intro.md): Introduction to Project Haystack

# Data Model

- [Kinds](Kinds.md): Data types
- [TimeZones](TimeZones.md): Respresentation of timezones
- [Units](Units.md): Respresentation  of units of measurement
- [Filters](Filters.md): Predicates for querying Haystack data

# Ontology

- [Ontology](Ontology.md): Overview of ontology
- [Sites](Sites.md): Sites entities
- [Systems](Systems.md): Systems entities
- [Synthetics](Synthetics.md): Synthetic data
- [Spaces](Spaces.md): Space entities including floors, rooms, and zones
- [Equips](Equips.md): Equipment entities
- [Points](Points.md): Point entities - sensors, actuators, and setpoints
- [Weather](Weather.md): Weather station entities
- [Devices](Devices.md): Device entities including controllers and networks

# Verticals

- [Meters](Meters.md): Modeling meters
- [AHUs](AHUs.md): Modeling air handling units
- [VAVs](VAVs.md): Modeling variable air volume terminal units
- [Zones](Zones.md): Modeling zone occupancy, hvac, air quality, lighting
- [Plants](Plants.md): Modeling central plants
- [Motors](Motors.md): Modeling fans, pumps, and other motors
- [ElecPanels](ElecPanels.md): Modeling electrical panels, circuits, and breakers
- [EVSE](EVSE.md): Modeling of Electric Vehicle Supply Equipment (EVSE)
- [VRF](VRF.md): Modeling variable refrigerant flow systems
- [ATES](ATES.md): Modeling aquifer thermal energy storage
- [DataCenters](DataCenters.md): Modeling data center spaces

# Filetype Formats

- [Filetypes](Filetypes.md): Data encoding formats
- [Zinc](Zinc.md): Zinc plain text format
- [Json](Json.md): JSON format
- [Trio](Trio.md): Tag record I/O format
- [Csv](Csv.md): Comma separated valus format

# HTTP API

- [HttpApi](HttpApi.md): Haystack HTTP API
- [Ops](Ops.md): HTTP API operations
- [Auth](Auth.md): HTTP API authentication

# Appendix

- [Tools](Tools.md): Tools to work with Haystack data
- [ChangeLog](ChangeLog.md): Version history
- [Changes3to4](Changes3to4.md): Notes for upgrading from version 3 to 4
- [License](License.md): Academic Free License 3.0

