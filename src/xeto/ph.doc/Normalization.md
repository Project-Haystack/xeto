...
ordered steps:
  ...
  6. **[Normalize Tags](#normalize-tags)**: normalize def tags
  7. **[Inherit](#inherit)**: recursively apply supertype tags into subtypes
  8. **[Validation](#validation)**: perform additional sanity checks
...
its [lib namespace](Namespaces#lib-namespaces):
...
derive its supertypes based on each def's `is` tag.  Specifically:
  ...
  2. infer the `is` on feature [key](Defs#feature-keys) defs
...
def identified by the [ph::PhEntity.defx] tag.  The defx mechanism provides for late
...
another defx.  The exception to this rule is tags annotated as [ph::PhEntity.accumulate]
...
This step normalizes specific tags according to the following rules:
  ...
  1. Add the inferrred [ph::PhEntity.lib] tag to each def

  2. Normalize def tags which subtype from list

Every def has an inferred [ph::PhEntity.lib] tag that is a reference to its declaring
...
normalize any tag that subtypes from [sys::List].  Example:
...
supertypes:
  ...
  6. If the tag is marked as [ph::PhEntity.accumulate] then inheritance is aggregated.
...
errors not caught in previous steps:
  ...
  5. Verify no defs declare a [ph::PhEntity.computedFromReciprocal] tag
