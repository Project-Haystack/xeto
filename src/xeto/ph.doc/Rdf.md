...
For example, [ph::PhEntity.site] is defined in the `phIoT` library. That library
...
# Marker Tags
Defs for marker tags are subtypes of [sys::Marker] via the [is] tag. See the
section on [Subtyping] for details.
...
Using these rules, the [ph::PhEntity.site] def now becomes
...
in Haystack are subtypes of the [ph::PhEntity.val] def. Scalar data types ([ph::PhEntity.scalar])
are further refined as subtypes of [ph::PhEntity.val]. This section describes the rules
for defining scalar data types in the RDF export.

Only *direct* subtypes of [ph::PhEntity.scalar] are defined as data types. They are
...
subtyping information (see [Marker Tags](#marker-tags) section).
...
# Value Tags
A value tag def is any tag def that is **not** a subtype of [sys::Marker].

If the tag def is a subtype of [sys::Ref] or is a [ph::PhEntity.choice], then declare the def to
be an instance of `owl:ObjectProperty`. All other value tags are declared to
be an instance of `owl:DatatypeProperty`.

If the tag def (or one of its [ph::PhEntity.defx] extensions) declares a [tagOn] association,
then specify the `rdfs:domain` to be all referent entities.

If the tag def is a [sys::Ref] or [ph::PhEntity.choice], then specify the `rdfs:range` to the value
of the [ph::PhEntity.of] tag on the def.

If the def is a basic [Tag Def](Defs#tag-defs) (e.g. [ph::PhEntity.tz]), then specify the `rdfs:range`
...
with an [sys::Entity.id] tag. The following outlines the high-level rules for exporting
