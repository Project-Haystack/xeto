...
There are two fundamental types of specs: *scalars* and *dicts*.
  - [**Scalars**](#scalars) define an atomic type encoded as a string.
    Common scalars include Str, Number, Date, Time, and DateTime.
  - [**Dicts**](#dicts) define a compound type composed of zero or
    more *slots* that are the data fields.

# Names

Specs are always defined within a [lib](Libs.md).  User defined spec
names must obey the following restrictions:
  - Must contain only ASCII upper case letters, ASCII lower case letters, digits, or underbar
